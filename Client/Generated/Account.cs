...
namespace visma
{
    ...
    /// Account operations.
    /// </summary>
    public partial class Account : IServiceOperations<VismanetAPI>, IAccount
    {
        /// <summary>
        ...
        /// </param>
        public Account(VismanetAPI client)
        {
            if (client == null) 
            {
        ...
        public async Task<HttpOperationResponse<IList<AccountDto>>> GetAllWithHttpMessagesAsync(string greaterThanValue = default(string), int? numberToRead = default(int?), int? skipRecords = default(int?), string orderBy = default(string), string lastModifiedDateTime = default(string), string lastModifiedDateTimeCondition = default(string), Dictionary<string, List<string>> customHeaders = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var _url = new Uri(new Uri(_baseUrl + (_baseUrl.EndsWith("/") ? "" : "/")), "controller/api/v1/account").ToString();
            ...
            // Create Result
            var _result = new HttpOperationResponse<IList<AccountDto>>();
            ...
            if ((int)_statusCode == 200)
            {
                ...
                try
                {
                    _result.Body = SafeJsonConvert.DeserializeObject<IList<AccountDto>>(_responseContent, this.Client.DeserializationSettings);
                }
        ...
        /// The cancellation token.
        /// </param>
        public async Task<HttpOperationResponse<AccountDto>> GetWithHttpMessagesAsync(string accountCd, Dictionary<string, List<string>> customHeaders = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (accountCd == null)
            {
                throw new ValidationException(ValidationRules.CannotBeNull, "accountCd");
            }
            // Tracing
            bool _shouldTrace = ServiceClientTracing.IsEnabled;
            string _invocationId = null;
            if (_shouldTrace)
            {
                _invocationId = ServiceClientTracing.NextInvocationId.ToString();
                Dictionary<string, object> tracingParameters = new Dictionary<string, object>();
                tracingParameters.Add("accountCd", accountCd);
                tracingParameters.Add("cancellationToken", cancellationToken);
                ServiceClientTracing.Enter(_invocationId, this, "Get", tracingParameters);
            }
            // Construct URL
            var _baseUrl = this.Client.BaseUri.AbsoluteUri;
            var _url = new Uri(new Uri(_baseUrl + (_baseUrl.EndsWith("/") ? "" : "/")), "controller/api/v1/account/{accountCd}").ToString();
            _url = _url.Replace("{accountCd}", Uri.EscapeDataString(accountCd));
            // Create HTTP transport objects
            HttpRequestMessage _httpRequest = new HttpRequestMessage();
            HttpResponseMessage _httpResponse = null;
            _httpRequest.Method = new HttpMethod("GET");
            ...
            // Create Result
            var _result = new HttpOperationResponse<AccountDto>();
            _result.Request = _httpRequest;
            _result.Response = _httpResponse;
            // Deserialize Response
            if ((int)_statusCode == 200)
            {
                ...
                try
                {
                    _result.Body = SafeJsonConvert.DeserializeObject<AccountDto>(_responseContent, this.Client.DeserializationSettings);
                }
                catch (JsonException ex)
                {
                    _httpRequest.Dispose();
