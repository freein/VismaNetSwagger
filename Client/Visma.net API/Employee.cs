...
namespace visma
{
    ...
    /// Employee operations.
    /// </summary>
    public partial class Employee : IServiceOperations<VismanetAPI>, IEmployee
    {
        /// <summary>
        ...
        /// </param>
        public Employee(VismanetAPI client)
        {
            if (client == null) 
            {
        ...
        public async Task<HttpOperationResponse<IList<EmployeeDto>>> GetAllEmplyeeWithHttpMessagesAsync(string greaterThanValue = default(string), int? numberToRead = default(int?), int? skipRecords = default(int?), string orderBy = default(string), string lastModifiedDateTime = default(string), string lastModifiedDateTimeCondition = default(string), Dictionary<string, List<string>> customHeaders = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            if (_shouldTrace)
            {
                ...
                ServiceClientTracing.Enter(_invocationId, this, "GetAllEmplyee", tracingParameters);
            }
            // Construct URL
            var _baseUrl = this.Client.BaseUri.AbsoluteUri;
            var _url = new Uri(new Uri(_baseUrl + (_baseUrl.EndsWith("/") ? "" : "/")), "controller/api/v1/employee").ToString();
            ...
            if ((int)_statusCode == 200)
            {
                ...
                try
                {
                    _result.Body = SafeJsonConvert.DeserializeObject<IList<EmployeeDto>>(_responseContent, this.Client.DeserializationSettings);
                }
        ...
        /// The cancellation token.
        /// </param>
        public async Task<HttpOperationResponse<EmployeeDto>> GetEmployeeWithHttpMessagesAsync(string employeeCd, Dictionary<string, List<string>> customHeaders = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (employeeCd == null)
            {
                throw new ValidationException(ValidationRules.CannotBeNull, "employeeCd");
            }
            // Tracing
            bool _shouldTrace = ServiceClientTracing.IsEnabled;
            string _invocationId = null;
            if (_shouldTrace)
            {
                _invocationId = ServiceClientTracing.NextInvocationId.ToString();
                Dictionary<string, object> tracingParameters = new Dictionary<string, object>();
                tracingParameters.Add("employeeCd", employeeCd);
                tracingParameters.Add("cancellationToken", cancellationToken);
                ServiceClientTracing.Enter(_invocationId, this, "GetEmployee", tracingParameters);
            }
            // Construct URL
            var _baseUrl = this.Client.BaseUri.AbsoluteUri;
            var _url = new Uri(new Uri(_baseUrl + (_baseUrl.EndsWith("/") ? "" : "/")), "controller/api/v1/employee/{employeeCd}").ToString();
            _url = _url.Replace("{employeeCd}", Uri.EscapeDataString(employeeCd));
            // Create HTTP transport objects
            HttpRequestMessage _httpRequest = new HttpRequestMessage();
            HttpResponseMessage _httpResponse = null;
            _httpRequest.Method = new HttpMethod("GET");
            ...
            if ((int)_statusCode == 200)
            {
                ...
                try
                {
                    _result.Body = SafeJsonConvert.DeserializeObject<EmployeeDto>(_responseContent, this.Client.DeserializationSettings);
                }
                catch (JsonException ex)
                {
                    _httpRequest.Dispose();
